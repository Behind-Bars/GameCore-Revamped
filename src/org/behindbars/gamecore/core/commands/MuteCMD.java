	...
	public boolean execute(CommandSender sender, String commandLabel, String[] args) {
		...
		if (Main.getPlayerHandler(player).getRank() < 8) {
	        ...
	        if (target == null) {
	        	...
	        	Bukkit.getServer().broadcastMessage(Main.getColorHandler().antiCheat + "Muted " + target.getName() + " for " + String.join(" ", Arrays.copyOfRange(args, 2, args.length)));
	        	Main.getPlayerHandler(target).setMuteDate("Muted By: " + player.getName() + " for " + String.join(" ", Arrays.copyOfRange(args, 2, args.length)));
	        }
        }
		return true;
	}
